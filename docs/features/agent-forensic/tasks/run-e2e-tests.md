...
- Runs spec files from `tests/e2e/features/agent-forensic/`
- Writes `tests/e2e/features/agent-forensic/results/latest.md` (PASS or FAIL)
- Writes `results/test-results.json` (structured JSON output from Playwright)

## Reference Files

- `tests/e2e/features/agent-forensic/` — Test scripts generated by T-test-2
- `tests/e2e/features/agent-forensic/results/latest.md` — Output: test results report
- `tests/e2e/config.yaml` — Test environment config (optional for CLI-only projects)

## Acceptance Criteria

- [ ] `tests/e2e/features/agent-forensic/results/latest.md` exists
...
2. Read `tests/e2e/features/agent-forensic/results/latest.md` to determine outcome
3. Run: `just test-e2e --feature agent-forensic`
...
- For each distinct root cause, create a fix task:
  ...
  task add --template fix-task \
           ...
           --var TEST_SCRIPT="tests/e2e/features/agent-forensic/<failing-spec>.spec.ts" \
           --var TEST_RESULTS="tests/e2e/features/agent-forensic/results/latest.md" \
