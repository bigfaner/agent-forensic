...
Read each record file from `docs/features/agent-forensic/tasks/records/` whose filename starts with `2.` and does NOT contain `.summary`. Exclude the phase summary's own prior record if one exists.
...
- All phase task records: `docs/features/agent-forensic/tasks/records/2.*.md`
