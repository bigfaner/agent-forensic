...
Read each record file from `docs/features/agent-forensic/tasks/records/` whose filename starts with `4.` and does NOT contain `.summary`. Exclude the phase summary's own prior record if one exists.
...
- All phase task records: `docs/features/agent-forensic/tasks/records/4.*.md`
