...
- Runs spec files from `tests/e2e/features/deep-drill-analytics/`
- Writes `tests/e2e/features/deep-drill-analytics/results/latest.md` (PASS or FAIL)
- Writes `results/test-results.json` (structured JSON output from Playwright)

## Reference Files

- `tests/e2e/features/deep-drill-analytics/` — Test scripts generated by T-test-2
- `tests/e2e/features/deep-drill-analytics/results/latest.md` — Output: test results report
- `tests/e2e/config.yaml` — Test environment config

## Acceptance Criteria

- [ ] `tests/e2e/features/deep-drill-analytics/results/latest.md` exists
...
2. Read `tests/e2e/features/deep-drill-analytics/results/latest.md` to determine outcome
3. Run: `just test-e2e --feature deep-drill-analytics`
...
- If **>30% of UI tests failed**: run app health diagnostics first
  ...
  2. If app is broken: create **ONE fix task** for the root cause
...
- For each distinct root cause, create a fix task:
  ...
  task add --template fix-task \
           ...
           --var TEST_SCRIPT="tests/e2e/features/deep-drill-analytics/<failing-spec>.spec.ts" \
           --var TEST_RESULTS="tests/e2e/features/deep-drill-analytics/results/latest.md" \
