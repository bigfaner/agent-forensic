...
Read each record file from `docs/features/deep-drill-analytics/tasks/records/` whose filename starts with `1.` and does NOT contain `.summary` (e.g., `1.1-*.md`, `1.2-*.md`, `1.3-*.md`, `1.4-*.md`). Exclude the phase summary's own prior record if one exists.
...
- All phase task records: `docs/features/deep-drill-analytics/tasks/records/1.*.md`
