...
Read each record file from `docs/features/deep-drill-analytics/tasks/records/` whose filename starts with `2.` and does NOT contain `.summary` (e.g., `2.1-*.md`, `2.2-*.md`, etc.). Exclude the phase summary's own prior record if one exists.
...
- 2.5: {{one-line summary from that task's record}}
- 2.6: {{one-line summary from that task's record}}
...
- All phase task records: `docs/features/deep-drill-analytics/tasks/records/2.*.md`
