...
- `tests/e2e/.graduated/deep-drill-analytics` — Graduation marker from T-test-4
...
**On failure**:
- Read Playwright output for failure details
- Analyze each failure: is it a code bug, test script issue, or environment issue?
- Run `task template fix-task` to view the fix-task template
- For each distinct root cause, create a fix task:
  ...
  ```
- When you record this task, the CLI will auto-downgrade `status: "completed"` to `status: "blocked"` if `testsFailed > 0`
