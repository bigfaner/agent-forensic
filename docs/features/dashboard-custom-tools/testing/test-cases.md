...
> **Note**: This feature is a terminal TUI application. All test cases are classified as CLI type. No UI (browser) or API (HTTP) interfaces exist.
...
> **Note**: Integration test count is a subset of CLI count. The integration test verifies the custom tools block is correctly wired into the dashboard panel.
...
- **Steps**:
  1. Launch agent-forensic and select the prepared session
...
- **Expected**: The "自定义工具" block is completely absent from the dashboard; the dashboard appearance matches the version without this feature
- **Priority**: P0
...
- **Steps**:
  ...
  2. Launch agent-forensic and select the prepared session
  3. Navigate to the dashboard panel
  4. Observe the "自定义工具" block layout
- **Expected**: The block renders in single-column stacked order: Skill section first, then MCP section, then Hook section; all content is fully readable with no horizontal truncation
- **Priority**: P0
...
- **Element**: `sitemap-missing`
- **Steps**:
  1. Ensure terminal width is ≥ 80 columns
  2. Launch agent-forensic and select the prepared session
  3. Navigate to the dashboard panel
  4. Observe the "自定义工具" block layout
...
- **Element**: `sitemap-missing`
- **Steps**:
  1. Launch agent-forensic and select a session with custom tool data
  2. Navigate to the dashboard panel
...
- **Expected**: The "自定义工具" block appears at the correct position in the dashboard and displays data correctly
- **Priority**: P0
